...
jQuery('.p-header_hamburger , .p-header_nav-background').on('click' , function(e){
	...
	jQuery('a[href^="#"]').click(function() {
		let header = jQuery(".j-header").height();
		let speed = 500;
		let id = jQuery(this).attr("href");
		let target = jQuery("#" == id ? "html" : id);
		let position = jQuery(target).offset().top - header;
		if ("fixed" !== jQuery(".j-header").css("position")) {
...
var show = jQuery(".p-mv,.p-page-teachers__wrapper,.p-page-gallery__subtitles,.p-page-information__wrapper,.p-contact-top__buttons,.p-page-single-mv").offset();
